<%-- 
    Document   : actualizarEjercicioA
    Created on : Jun 11, 2021, 12:04:06 AM
    Author     : Emiliano
--%>

<%@page import="Modelo.CIntensidad"%>
<%@page import="Modelo.MEjerciciosA"%>
...
<html>
    ...
    <link href="serverYoulev/web/css/asignarregimen.css" rel="stylesheet" type="text/css">
    <link href="serverYoulev/web/css/estilos.css" rel="stylesheet" type="text/css">
        ...
        <div class="contenedor-inputs">
          ...
          <select name="ejercicio" class="combobox" required>
                            ...
                            for(CEjercicios ejer : listaEjer){
                                if(objEjer.getId_ejer()==ejer.getId()){
                                    %>
                                    <option value="<%=ejer.getId()%>" selected="true"><%=ejer.getNombre()%></option>
                                    <%
                                }else{
                                    %>
          ...
          <select name="intensidad" class="combobox" required>
                            ...
                            for(CIntensidad inten : listaIntensidades){
                                if(objEjer.getId_inten()==inten.getId()){
                                    %>
                                    <option value="<%=inten.getId()%>" selected="true"><%=inten.getDescr()%></option>
                                    <%
                                }else{
                                    %>
          ...
          <input type="text" value="<%=objEjer.getRep()%>" name="repeticiones" placeholder="Repeticiones" required>
          <br>
          <br>
          <label id="titulo">Numero de series</label>
          <input type="number" value="<%=objEjer.getNumS()%>" name="numseries" placeholder="Numero de series" required>
          <br>
          <br>
          <label id="titulo">Observaciones</label>
          <textarea name="observaciones" placeholder="Observaciones" cols="30" rows="1" required><%=objEjer.getObs()%></textarea>
          <br>
          <br>
